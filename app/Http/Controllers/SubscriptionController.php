...
class SubscriptionController extends Controller
{
    ...
    {
        $sub= DB::table('subscriptions')->where('id', $r->id)->first();
        $employer = DB::table('employers')->where('id', $sub->employer_id)->first();
        if($r->status==0)
        {
            DB::table('subscriptions')->where('id', $r->id)->update(['status'=>1]);
            // send email to employer about de-active subscription
            $sms = "<h3>Subscription Information.</h3><hr>";
            $sms .= "<p>Your subscription is approved. You are ready to go with us!</p>";
            $sms .= "For more information, please visit the link here: <a href='http://masterjobscambodia.com'>masterjobscambodia.com</a>";
            Right::sms($employer->email, "Activate Subscription Information", $sms);
            ...
            $sms = "<h3>Subscription Information.</h3><hr>";
            $sms .= "<p>Your subscription is de-activate successfully.</p>";
            Right::sms($employer->email, "De-activate Subscription Information", $sms);
