...
class FrontController extends Controller
{
    ...
    public function index()
    {
        ...
        $total = $count->total + 5;
        DB::table('counters')->where('id',1)->update(['total'=>$total]);
    ...
    public function category($id)
    {
        ...
        $data['jobs'] = DB::table('jobs')
            ...
            ->select("jobs.id", "jobs.job_title", "jobs.location", "jobs.create_at", "categories.name as catname", "jobs.closing_date", "companies.name as cname")
    ...
    public function detail($id)
    {
        ...
        $data['job'] = DB::table("jobs")
            ...
            ->join("categories", "jobs.category_id", "categories.id")
            ->where("jobs.id", $id)
            ->select("jobs.*", "employers.first_name", "employers.last_name", "employers.email", "employers.phone", "categories.name as catname")
            ->first();
        $data['company'] = DB::table('companies')->where('employer_id', $data['job']->employer_id)->first();
